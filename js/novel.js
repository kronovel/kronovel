...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showPage(page) {
    ...
    // Update page number display
    document.getElementById("page-nomor").textContent = `Page ${currentPage}`;
  ...
  function updatePaginationControls() {
    const prevPageButton = document.getElementById("prev-tombol");
    const nextPageButton = document.getElementById("next-tombol");
  ...
  function setupPagination() {
    document.getElementById("prev-tombol").addEventListener("click", () => {
      if (currentPage > 1) showPage(currentPage - 1);
    });

    document.getElementById("next-tombol").addEventListener("click", () => {
      if (currentPage < totalPages) showPage(currentPage + 1);
    });
